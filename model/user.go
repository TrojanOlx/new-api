...
type User struct {
	...
	APIIPMode             string                     `json:"-" gorm:"type:varchar(16);column:api_ip_mode"`
	APIIPAllowlist        string                     `json:"-" gorm:"type:text;column:api_ip_allowlist"`
	DevicePolicyMode      string                     `json:"-" gorm:"type:varchar(16);column:device_policy_mode"`
	DeviceControlsEnabled bool                       `json:"-" gorm:"column:device_controls_enabled"`
	AccessPolicyVersion   int64                      `json:"-" gorm:"type:bigint;not null;default:1;column:access_policy_version"`
	AdminPermissions      map[string]map[string]bool `json:"admin_permissions,omitempty" gorm:"-:all"`
}

func (user *User) ToBaseUser() *UserBase {
	accessPolicyVersion := user.AccessPolicyVersion
	if accessPolicyVersion < 1 {
		accessPolicyVersion = 1
	}
	apiIPMode := user.APIIPMode
	if apiIPMode == "" {
		apiIPMode = string(constant.UserIPPolicyUnrestricted)
	}
	apiIPAllowlist := user.APIIPAllowlist
	if apiIPAllowlist == "" {
		apiIPAllowlist = "[]"
	}
	devicePolicyMode := user.DevicePolicyMode
	if devicePolicyMode == "" {
	...
	cache := &UserBase{
		...
		APIIPMode:             apiIPMode,
		APIIPAllowlist:        apiIPAllowlist,
		DevicePolicyMode:      devicePolicyMode,
		DeviceControlsEnabled: user.DeviceControlsEnabled,
