/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import type { GetUserDevicesParams } from '../types'

export const userAccessPolicyQueryKeys = {
  all: ['users'] as const,
  user: (userId: number) => [...userAccessPolicyQueryKeys.all, userId] as const,
  policy: (userId: number) =>
    [...userAccessPolicyQueryKeys.user(userId), 'access-policy'] as const,
  trustedDevicePrerequisite: (userId: number) =>
    [...userAccessPolicyQueryKeys.user(userId), 'trusted-device'] as const,
  deviceLists: (userId: number) =>
    [...userAccessPolicyQueryKeys.user(userId), 'devices'] as const,
  devices: (userId: number, filters: GetUserDevicesParams = {}) =>
    [...userAccessPolicyQueryKeys.deviceLists(userId), filters] as const,
  device: (userId: number, deviceId: number) =>
    [...userAccessPolicyQueryKeys.deviceLists(userId), deviceId] as const,
}
